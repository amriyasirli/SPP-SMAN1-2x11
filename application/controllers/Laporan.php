...
class Laporan extends CI_Controller {
    ...
    public function filterBulan()
    {
        ...
        $siswa = $this->db->join('tbl_kelas', 'tbl_kelas.id_kelas=tbl_siswa.id_kelas')
                            // ->join('tbl_kelas', 'tbl_kelas.id_kelas=tbl_pembayaran.id_kelas')
                            ->order_by('tbl_siswa.nama_kelas', 'asc')
                            ->order_by('tbl_siswa.nama_siswa', 'asc')
                            // ->group_by('tbl_pembayaran.id_siswa')
                            ->get('tbl_siswa')
                            ->result();
        ...
        $pdf->AddPage();
            ...
            $pdf->Image(base_url('assets/img/tutwuri.png'),268,6,30,0,'PNG');
            $pdf->SetFont('Arial','B',14);
            $pdf->Cell(0,5,'DINAS PENDIDIKAN PASAMAN BARAT',0,1,'C');
            // $pdf->Cell(0,5,'',0,1,'C');
            $pdf->Cell(0,5,'SMA NEGERI 1 2x11 ENAM LINGKUNG',0,1,'C');
            // $pdf->Line(10,$this->GetY(),100,$this->GetY());
            $pdf->SetFont('Arial','',8);
            $pdf->Cell(0,3,' JL. Bari Sicincin',0,1,'C');
            $pdf->Cell(0,3,'Sicincin, Pasaman Barat, Sumatera Barat 25584 ',0,1,'C');
            ...
            $pdf->Cell(0,5,'LAPORAN PEMBAYARAN SPP BULAN ',0,1,'C');
        ...
        $pdf->Cell(230);
        $pdf->Cell(0,5,'Kepala Sekolah,',0,1,'M');
        $pdf->Cell(10,10,'',0,1);
        $pdf->Cell(10,10,'',0,1);
        $pdf->Cell(230);
        $pdf->SetFont('Times','B',12);
        $pdf->Cell(0,5,'Sri Astuti',0,1,'L');
        $pdf->SetFont('Times','',12);
        $pdf->Cell(230);
